...
import { haversineDistance, bearing, movePoint, knotsToKmPerSecond, pointInPolygon, distanceAlongPath } from './geo.js';
import { computeRoute } from './routing.js';
import { batchWeatherUpdate } from './weather.js';
...
const WEATHER_UPDATE_INTERVAL = 60;

export class FleetSimulation {
  constructor(fleetPath) {
    const data = JSON.parse(readFileSync(fleetPath, 'utf-8'));
    this.scenario = data.scenario;
    this.navigableWater = data.navigableWater;
  ...
  start() {
    ...
    this.weatherInterval = setInterval(() => this.updateWeather(), WEATHER_UPDATE_INTERVAL * 1000);
    this.updateWeather();
  }
  ...
  async updateWeather() {
    ...
    const results = await batchWeatherUpdate(ships);
    for (const [id, w] of results) {
      this.weatherData.set(id, w);
      const ship = this.ships.get(id);
      if (ship) ship.weather = w;
    }
  }
  ...
  tick() {
    ...
    for (const ship of this.ships.values()) {
      ...
      const weather = this.weatherData.get(ship.shipId);
      const adverseWeather = weather?.adverse || false;
      ...
      for (const zone of this.restrictedZones) {
        if (pointInPolygon(ship.position[0], ship.position[1], zone.polygon)) {
          if (!ship._inZone?.has(zone.id)) {
            if (!ship._inZone) ship._inZone = new Set();
    ...
    for (let i = 0; i < shipList.length; i++) {
      for (let j = i + 1; j < shipList.length; j++) {
        ...
        if (d < PROXIMITY_WARNING_KM) {
          ...
          if (!this._proximityActive) this._proximityActive = new Set();
  ...
  rerouteShip(ship) {
    ...
    if (route) {
      ...
      this.alerts.push(this.createAlert('stranded', 'critical', ship.shipId,
        `${ship.name} is stranded - no valid route to ${dest.name}`));
      this.broadcast({ type: 'alerts', data: [this.alerts[this.alerts.length - 1]] });
    }
  }
  ...
  acknowledgeAlert(alertId) {
    ...
    if (alert) {
      ...
      this.broadcast({ type: 'alert_acknowledged', data: alert });
    }
    return alert;
  }
  ...
  saveSnapshot() {
    const snapshot = {
      ...
      alerts: this.alerts.filter(a => a.timestamp > Date.now() - 3600000),
    };
